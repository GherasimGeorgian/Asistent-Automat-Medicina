...
train_dir = 'C:/Users/Ghera/Desktop/segmentation/' # directory of training images
pretrained_model_path = 'C:/Users/Ghera/source/repos/new_segm/source/ResUNetBladder.h5' # path of pretrained model
model_save_path = './ResUNetBladderY.h5' # path of model to save
train_image_dir = os.path.join(train_dir, 'realdata/original') # 
...
val_size = .50 # split of training set between train and validation set
...
train_df = pd.read_csv(os.path.join(train_dir, 'bladder_mini18.csv')).fillna(-1)
...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, list_ids, labels, image_dir, batch_size=1,
                 img_h=64, img_w=64, shuffle=True):
    ...
    def __data_generation(self, list_ids_temp):
        ...
        y = np.empty((self.batch_size, 64, 64, 2))
        
        for idx, id in enumerate(list_ids_temp):
            file_path =  os.path.join(self.image_dir, id)
            correct_path = self.image_dir + '/'+id
            #print(correct_path)
            #print("path:",file_path)
            #print("x1",self.image_dir)
            #print("x2",correct_path)
            
            image = cv2.imread(correct_path, cv2.IMREAD_GRAYSCALE)
            #print(image,"hmm")
            ...
            #--------------------
            img = Image.open(correct_path) #for example image size : 28x28x3
            img = img.resize((64, 64), Image.ANTIALIAS)
            img1 = img.convert('L')  #convert a gray scale
            #print(img1.size)
            y_result = np.expand_dims(img1, axis=-1)
            ...
            #image_resized -= image_resized.mean().astype('int32')
            
            #image_resized /= image_resized.std()
            
            #img_aux = image_resized
            image_resized /= image_resized.std()
            #image_resized = img_aux.astype('int32')
            
            mask = np.empty((64, 64, 2))
            
            for idm, image_class in enumerate(['1','2']):
                rle = self.labels.get(id + '_' + image_class)
                # if there is no mask create empty mask
                if rle is None:
                    class_mask = np.zeros((64, 64))
                else:
                    class_mask = rle_to_mask(rle, width=100, height=100)
             
                class_mask_resized = cv2.resize(class_mask, (64, 64))
                mask[...,idm] = class_mask_resized
            
            X[idx,] = np.expand_dims(image_resized, axis=0)
            y[idx,] = mask
...
adam = tf.keras.optimizers.Adam(lr = 0.05, epsilon = 0.2)
...
test_files = [f for f in glob.glob('C:/Users/Ghera/Desktop/segmentation/test_images_bladder/' + "*.jpeg", recursive=True)]
